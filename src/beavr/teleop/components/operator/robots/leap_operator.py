...
class LeapHandOperator(Operator):
    def __init__(
        ...
        self.thread_pool = ThreadPoolExecutor(
            max_workers=1
        )  # Single thread as PyBullet handles all fingers at once
    ...
    def _apply_retargeted_angles(self):
        ...
        try:
            ...
            for finger_type in ["thumb", "index", "middle", "ring"]:
                if (
                    not self.finger_configs[f"freeze_{finger_type}"]
                    and not self.finger_configs[f"no_{finger_type}"]
                ):
                ...
                if not self.log_queue.full():  # Skip logging if queue is full rather than blocking
                    log_data = {
                        "finger_input_positions": {
                            k: v["input_positions"].copy()
                            if isinstance(v["input_positions"], np.ndarray)
                            else v["input_positions"]
                            for k, v in finger_data.items()
                        },
                        "finger_computed_angles": {
                            k: v["computed_angles"].copy()
                            if isinstance(v["computed_angles"], np.ndarray)
                            else v["computed_angles"]
                            for k, v in finger_data.items()
                        },
                        "finger_states": desired_joint_angles.copy()
                        if isinstance(desired_joint_angles, np.ndarray)
                        else desired_joint_angles,
                    }
...
def allegro_to_leap_hand(joints, teleop=True, zeros=True):

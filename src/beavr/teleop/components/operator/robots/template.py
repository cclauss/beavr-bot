...
class TemplateArmOperator(Operator):
    def __init__(
        ...
        # Gripper Publisher
        self.gripper_publisher = ZMQPublisherManager(host=host, port=gripper_port)
        # Cartesian Publisher
        self.cartesian_publisher = ZMQPublisherManager(host=host, port=cartesian_publisher_port)
        # Joint Publisher
        self.joint_publisher = ZMQPublisherManager(host=host, port=joint_publisher_port)
        # Cartesian Command Publisher
        self.cartesian_command_publisher = ZMQPublisherManager(
    ...
    def _get_scaled_cart_pose(self, moving_robot_homo_mat):
        ...
        self.robot_pose_aa_to_affine(home_pose_array)
    ...
    def _reset_teleop(self):
        ...
        logger.info("Resetting complete")
    ...
    def _apply_retargeted_angles(self, log=False):
        ...
        self.gripper_publisher.publish(self.gripper_correct_state, "gripper_right")
        position = self.robot.get_cartesian_position()
        joint_position = self.robot.get_joint_position()
        self.cartesian_publisher.publish(position, "cartesian")
        self.joint_publisher.publish(joint_position, "joint")
        self.cartesian_command_publisher.publish(final_pose, "cartesian")

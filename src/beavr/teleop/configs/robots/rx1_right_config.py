...
class OculusVRHandDetectorCfg:
    ...
    def build(self):
        return OculusVRHandDetector(
            ...
            keypoint_pub_port=self.keypoint_pub_port,
            button_port=self.button_port,
            button_publish_port=self.button_publish_port,
            teleop_reset_port=self.teleop_reset_port,
            teleop_reset_publish_port=self.teleop_reset_publish_port,
        )
...
class RX1RightCfg:
    ...
    def build(self):
        return RX1Right(
            host=self.host,
            ee_pose_op_pub=self.ee_pose_op_pub,
            ee_pose_op_sub=self.ee_pose_op_sub,
            ee_pose_ros_pub=self.ee_pose_ros_pub,
            reset_op_sub=self.reset_op_sub,
            ee_pose_ros_sub=self.ee_pose_ros_sub,
            joint_state_ros_sub=self.joint_state_ros_sub,
        )
...
class Rx1RightConfig:
    ...
    transforms: list = field(
        default_factory=lambda: [
            TransformHandPositionCoordsCfg(
                host="10.31.152.148",
                keypoint_sub_port="${keypoint_port}",
                keypoint_transform_pub_port="${transformed_position_keypoint_port}",
    ...
    robots: list = field(
        default_factory=lambda: [
            RX1RightCfg(
                host="10.31.152.148",
                ee_pose_op_pub=10010,
                ee_pose_op_sub=10009,
                ee_pose_ros_pub=9118,
                reset_op_sub=9069,
                ee_pose_ros_sub=5555,
                joint_state_ros_sub=5556,
            )
        ]
    )
    operators: list = field(
        default_factory=lambda: [
            RX1RightOperatorCfg(
                ...
                stream_configs={"host": "10.31.152.148", "port": "10005"},
                stream_oculus=True,
                endeff_publish_port=10009,
                endeff_subscribe_port=10010,
                moving_average_limit=1,
                arm_resolution_port="8093",
                use_filter=False,
                teleoperation_reset_port="8102",
                reset_publish_port=9069,
                logging_config={
                    "enabled": False,
                    "log_dir": "logs",
                    "log_poses": True,
    ...
    recorded_data: list = field(
        default_factory=lambda: [["joint_states", "cartesian_states", "position", "velocity", "effort"]]
    )
